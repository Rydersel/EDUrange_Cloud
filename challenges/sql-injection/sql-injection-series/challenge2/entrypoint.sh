...
# Use environment variable for DB_NAME or default to our flag name
if [ -z "$DB_NAME" ]; then
...
# Ensure we have the FLAG environment variable
if [ -z "$FLAG" ]; then
    FLAG="CTF_error_based_injection_flag"
...
if mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD" -e "USE $DB_NAME" >/dev/null 2>&1; then
    ...
    if [ -z "$USER_COUNT" ] || [ "$USER_COUNT" -eq "0" ]; then
        ...
        -- Create flags table with the actual flag value
        ...
        -- Insert the flag from the environment variable
        INSERT INTO flags (flag_value, description) VALUES
        ('$FLAG', 'Congratulations! You found the flag!'),
        ('CTF_fake_flag_1', 'This is not the real flag!'),
        ('CTF_fake_flag_2', 'Nice try, but this is not the flag either!');"
        ...
        -- Insert the flag if the table was just created
        INSERT IGNORE INTO flags (flag_value, description) VALUES
        ('$FLAG', 'Congratulations! You found the flag!'),
        ('CTF_fake_flag_1', 'This is not the real flag!'),
        ('CTF_fake_flag_2', 'Nice try, but this is not the flag either!');"

        echo "Flags table check/creation complete."
    fi
else
