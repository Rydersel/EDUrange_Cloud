...
if [ -z "$TARGET_FLAG" ]; then
    ...
    echo "CTF{default-flag}" | base64 > /opt/.secret/encoded_flag.txt
else
    # Use the flag provided in the environment variable, encoded in base64
    echo "$TARGET_FLAG" | base64 > /opt/.secret/encoded_flag.txt
fi

# Ensure proper permissions
chmod 0400 /opt/.secret/encoded_flag.txt

# Create a false flag to mislead
echo "Nice try, but the real flag is elsewhere!" > /root/root.txt
...
The flag is not where you found it last time.
