...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch('/api/instance-manager-proxy?path=install-type', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to install challenge type' }, { status: 500 });
    }

    const result = await response.json();

    // Log the challenge type installation
    ...
    console.error('Error installing challenge type:', error);
