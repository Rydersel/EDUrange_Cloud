...
export default function InstanceComponent({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const isChallengePage = () => pathname === '/dashboard/challenge';

  // Modified toast function with improved throttling
  const showToastIfOnChallengePage = (title: string, description: string, variant: "default" | "destructive" = "default") => {
    ...
    const now = Date.now();
    const toastKey = `${title}-${variant}`;
    const lastTime = lastToastTime[toastKey] || 0;

    // Throttle all toasts - different intervals for different types
    const minInterval = variant === "destructive" ? 30000 : 10000; // 30s for errors, 10s for others

    if (now - lastTime < minInterval) {
      return;
    }

    setLastToastTime(prev => ({ ...prev, [toastKey]: now }));
  ...
  // Set up polling for instance status updates with exponential backoff
  useEffect(() => {
    ...
    const startPolling = () => {
      ...
      // Calculate polling interval based on consecutive errors
      const getPollingInterval = () => {
        if (consecutiveErrors === 0) return 5000; // Normal interval: 5 seconds
        const backoffTime = Math.min(30000, 5000 * Math.pow(2, consecutiveErrors - 1)); // Max 30 seconds
        return backoffTime;
      };
  ...
  }, [consecutiveErrors, pathname]); // Add pathname to dependencies
  ...
  const fetchInstances = async () => {
    ...
    try {
      ...
      if (!dbResponse.ok) {
        setConsecutiveErrors(prev => Math.min(prev + 1, 5)); // Cap at 5 to prevent excessive backoff
      ...
      // Reset consecutive errors on successful DB fetch
      ...
      try {
        ...
        if (imResponse.ok) {
          ...
          if (imData.instances && Array.isArray(imData.instances)) {
            // Merge data from both sources
            const mergedInstances = [];

            // First add all pods from instance manager with most up-to-date status
            for (const pod of imData.instances) {
              const dbInstance = dbInstances.get(pod.id);
              ...
              if (dbInstance) {
                ...
                dbInstances.delete(pod.id);
              } else {
                // This is a new pod not yet in the database, create a basic record
                mergedInstances.push({
                  id: pod.id,
                  userId: pod.userId,
                  userEmail: pod.userEmail || 'Loading...',
                  userName: pod.userName || 'Loading...',
                  challengeImage: pod.challengeImage,
                  challengeUrl: pod.challengeUrl,
                  creationTime: pod.creationTime,
                  status: pod.status,
                  flagSecretName: pod.flagSecretName,
                  groupId: pod.groupId,
                  groupName: pod.groupName || (pod.groupId === 'standalone' ? 'Standalone' : pod.groupId)
                });
              }
            }
      ...
      // On successful fetch, reset error counters
      setConsecutiveErrors(0);
      setLastErrorTime(0);
    ...
    <div className="space-y-4">
      ...
      {instances.length === 0 ? (
        <div className="border rounded-md p-6 text-center">
          ...
          <Link href="/admin/challenge/new" className="mt-4 inline-block">
